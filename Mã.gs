function doPost(e) {
  try {
    ...
    if (action === "loginSystem") {
      result = loginSystem(data.phone, data.pin);
    } else if (action === "getStudentDetailsForTutor") {
      result = getStudentDetailsForTutor(data.studentPhone, data.studentName);
    } else if (action === "guiPhanHoi") {
      result = guiPhanHoi(data.maHS, data.tenHocSinh, data.noiDung);
    } else if (action === "themHocSinhMoi") {
      result = themHocSinhMoi(data.tutorPhone, data.phuHuynhName, data.studentName, data.studentPhone, data.tuition);
    } else if (action === "suaThongTinHocSinh") {
      result = suaThongTinHocSinh(data.oldPhone, data.phuHuynhName, data.studentName, data.studentPhone, data.tuition);
    } else if (action === "themBuoiHoc") {
      result = themBuoiHoc(
        data.studentPhone, data.studentName, data.tuan, data.ngayDay,
        data.monHoc, data.noiDung, data.danhGiaBTVN, data.diemDauGio,
        data.diemDinhKi, data.trangThai
      );
    } else if (action === "capNhatThongTinGiaSu") {
      result = capNhatThongTinGiaSu(data.tutorPhoneCu, data.tenMoi, data.sdtMoi, data.pinMoi);
    } else if (action === "suaBuoiHoc") {
      result = suaBuoiHoc(
        data.rowIndex, data.tuan, data.ngayDay, data.monHoc,
        data.noiDung, data.danhGiaBTVN, data.diemDauGio, data.diemDinhKi, data.trangThai
      );
    } else if (action === "capNhatThoiKhoaBieu") {
      result = capNhatThoiKhoaBieu(
        data.tutorPhone, data.studentName, data.mon, data.tue,
        data.wed, data.thu, data.fri, data.sat, data.sun
      );
    } else if (action === "xoaHocSinhTamThoi") {
      result = xoaHocSinhTamThoi(data.tutorPhone, data.studentPhone);
    } else if (action === "khoiPhucHocSinh") {
      result = khoiPhucHocSinh(data.tutorPhone, data.studentPhone);
    } else if (action === "xoaBuoiHoc") {
      result = xoaBuoiHoc(data.rowIndex);
    } else if (action === "getTutorSchedule") {
      result = getTutorSchedule(data.tutorPhone);
    } else if (action === "capNhatNhieuDongHocPhi") {
      result = capNhatNhieuDongHocPhi(data.paidRowIndices, data.unpaidRowIndices);
    } else if (action === "capNhatDongHocPhiBuoiHoc") {
      result = capNhatDongHocPhiBuoiHoc(data.rowIndices);
    } else if (action === "adminLuuGiaSu") {
      result = adminLuuGiaSu(data.oldPhone, data.name, data.phone, data.pin, data.qrUrl);
    } else if (action === "adminLuuHocSinh") {
      result = adminLuuHocSinh(data.oldPhone, data.phuHuynhName, data.studentName, data.studentPhone, data.tuition, data.tutorPhone);
    } else if (action === "adminCapNhatTaiKhoan") {
      result = adminCapNhatTaiKhoan(data.oldPhone, data.name, data.phone, data.pin);
    } else if (action === "xoaGiaSuTamThoi") {
      result = xoaGiaSuTamThoi(data.tutorPhone);
    } else if (action === "khoiPhucGiaSu") {
      result = khoiPhucGiaSu(data.tutorPhone);
    } else if (action === "traCuuMaBaiTap") {
      result = traCuuMaBaiTap(data.maBt, data.sdtHs);
    } else if (action === "nopBaiTap") {
      result = nopBaiTap(data.maBt, data.tenHs, data.sdtHs, data.fileBase64, data.fileName, data.mimeType);
    } else if (action === "layLichSuNopBaiTrongNgay") {
      result = layLichSuNopBaiTrongNgay(data.sdtHs);
    } else if (action === "xoaBaiNopTamThoi") {
      result = xoaBaiNopTamThoi(data.rowIndex);
    } else if (action === "khoiPhucBaiNop") {
      result = khoiPhucBaiNop(data.rowIndex);
    } else if (action === "giaoBaiTap") {
      result = giaoBaiTap(data.maBt, data.monHoc, data.tenBaiTap, data.ngayPhatHanh, data.hanNop, data.fileBase64, data.fileName, data.mimeType);
    } else if (action === "layDanhSachHocSinhNop") {
      result = layDanhSachHocSinhNop(data.studentPhone);
    } else if (action === "layDanhSachBaiNopDaXoa") {
      result = layDanhSachBaiNopDaXoa(data.sdtHs);
    } else {
...
function traCuuDuLieuHocSinh(phone, hsRow, ss) {
  ...
  if (sheetLich && studentName) {
    var dataLich = sheetLich.getDataRange().getDisplayValues();
    var targetName = String(studentName).trim().toLowerCase();
    for (var i = 1; i < dataLich.length; i++) {
      if (String(dataLich[i][1]).trim().toLowerCase() === targetName) {
        ketQua.lichHoc = {
          tutorName: dataLich[i][2],
          mon: dataLich[i][3],
          tue: dataLich[i][4],
          wed: dataLich[i][5],
          thu: dataLich[i][6],
...
function adminLuuGiaSu(oldPhone, name, phone, pin, qrUrl) {
  var lock = LockService.getScriptLock();
  try {
    lock.waitLock(10000);
    var ss = SpreadsheetApp.getActiveSpreadsheet();
...
// ================= HỆ THỐNG GIAO & NỘP BÀI TẬP (MỚI) =================

// Hàm tải file lên Google Drive dưới dạng base64 và lấy link chia sẻ
function upLoadFileToDrive(fileBase64, fileName, mimeType) {
  var folderName = "Học sinh nộp bài";
...
// Tìm thông tin bài tập theo Mã bài tập
function traCuuMaBaiTap(maBt, sdtHs) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetGiao = ss.getSheetByName('Giao bài tập');
    if (!sheetGiao) return { error: "Không tìm thấy sheet 'Giao bài tập'" };
    
    var dataGiao = sheetGiao.getDataRange().getDisplayValues();
    var cleanMa = String(maBt).trim().toLowerCase();
    var foundBt = null;
    
    for (var i = 1; i < dataGiao.length; i++) {
      if (String(dataGiao[i][0]).trim().toLowerCase() === cleanMa) {
        foundBt = {
          maBt: dataGiao[i][0],
          monHoc: dataGiao[i][1],
          tenBaiTap: dataGiao[i][2],
          ngayPhatHanh: dataGiao[i][3],
          hanNop: dataGiao[i][4],
          linkDe: dataGiao[i][5]
        };
        break;
    ...
    if (!foundBt) {
      return { error: "Không tìm thấy bài tập nào khớp với mã: " + maBt.toUpperCase() };
    }
    
    // Tự động tìm tên học sinh nếu nhập số điện thoại
    var tenHocSinh = "";
    if (sdtHs) {
      var sheetHS = ss.getSheetByName('Mã học sinh');
      if (sheetHS) {
        var normPhone = normalizePhone(sdtHs);
        var dataHS = sheetHS.getDataRange().getDisplayValues();
        for (var j = 1; j < dataHS.length; j++) {
          if (normalizePhone(dataHS[j][3]) === normPhone) {
            tenHocSinh = dataHS[j][2];
            break;
          }
        }
      }
    ...
    return { error: "Lỗi tra cứu mã bài tập: " + e.toString() };
  }
}

// Ghi nhận nộp bài tập của học sinh
function nopBaiTap(maBt, tenHs, sdtHs, fileBase64, fileName, mimeType) {
  var lock = LockService.getScriptLock();
  try {
    lock.waitLock(15000);
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return { error: "Không tìm thấy sheet 'Học sinh nộp bài'" };
    
    var fileUrl = "";
    if (fileBase64) {
      fileUrl = upLoadFileToDrive(fileBase64, fileName, mimeType);
    }
    
    // Tìm tên môn và tên bài tập để ghép thành tên bài học
    var monHoc = "";
    var tenBaiTap = "";
    var sheetGiao = ss.getSheetByName('Giao bài tập');
    if (sheetGiao) {
    ...
    var tenBaiHoc = "[" + monHoc + "] " + tenBaiTap;
    
    // Tự động sinh STT
    var dataNop = sheetNop.getDataRange().getDisplayValues();
    var nextStt = 1;
    if (dataNop.length > 1) {
      var lastStt = parseInt(dataNop[dataNop.length - 1][0]);
      if (!isNaN(lastStt)) nextStt = lastStt + 1;
    }
    
    var now = new Date();
    var dateString = Utilities.formatDate(now, "GMT+7", "dd/MM/yyyy HH:mm:ss");
    
    // Lưu vào sheet: STT, Mã bài tập, Tên học sinh, SĐT học sinh, Tên bài học, Link bài làm, Thời gian nộp, Ngày xóa
    sheetNop.appendRow([nextStt, maBt, tenHs, "'" + sdtHs, tenBaiHoc, fileUrl, dateString, ""]);
    var lastRow = sheetNop.getLastRow();
    sheetNop.getRange(lastRow, 1, 1, 8).setFontFamily("Arial");
    
    return { success: true, link: fileUrl, msg: "Nộp bài thành công!" };
  } catch (e) {
    return { error: "Lỗi nộp bài tập: " + e.toString() };
  } finally {
    lock.releaseLock();
  }
}

// Lấy danh sách các bài tập đã nộp trong ngày (chỉ cho phép sửa/xóa khi còn trong ngày)
function layLichSuNopBaiTrongNgay(sdtHs) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return [];
    ...
    for (var i = 1; i < dataNop.length; i++) {
      var phone = normalizePhone(dataNop[i][3]);
      var deletedDate = dataNop[i][7] ? dataNop[i][7].trim() : "";
      var thoiGianNop = dataNop[i][6] ? dataNop[i][6].trim() : "";
      
      if (phone === normPhone && deletedDate === "" && thoiGianNop.startsWith(todayStr)) {
        list.push({
          rowIndex: i + 1,
          stt: dataNop[i][0],
          maBt: dataNop[i][1],
          tenHocSinh: dataNop[i][2],
          tenBaiHoc: dataNop[i][4],
          linkBaiLam: dataNop[i][5],
...
// Xóa bài nộp tạm thời (chuyển vào thùng rác)
function xoaBaiNopTamThoi(rowIndex) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return { error: "Không tìm thấy sheet 'Học sinh nộp bài'" };
    ...
    var r = parseInt(rowIndex);
    if (isNaN(r) || r < 2 || r > sheetNop.getLastRow()) {
      return { error: "Vị trí dòng cần xóa không hợp lệ." };
    }
    
    var now = new Date();
    var dateString = Utilities.formatDate(now, "GMT+7", "dd/MM/yyyy HH:mm:ss");
    sheetNop.getRange(r, 8).setValue(dateString).setFontFamily("Arial");
    return { success: true };
  } catch (e) {
    return { error: "Lỗi xóa bài nộp: " + e.toString() };
  }
}

// Khôi phục bài nộp từ thùng rác
function khoiPhucBaiNop(rowIndex) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return { error: "Không tìm thấy sheet 'Học sinh nộp bài'" };
    ...
    sheetNop.getRange(r, 8).setValue("").setFontFamily("Arial");
    return { success: true };
  } catch (e) {
    return { error: "Lỗi khôi phục bài nộp: " + e.toString() };
  }
}

// Giao bài tập mới (Gia sư)
function giaoBaiTap(maBt, monHoc, tenBaiTap, ngayPhatHanh, hanNop, fileBase64, fileName, mimeType) {
  var lock = LockService.getScriptLock();
  try {
    lock.waitLock(15000);
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    // Kiểm tra xem Mã bài tập đã tồn tại hay chưa
    var dataGiao = sheetGiao.getDataRange().getDisplayValues();
    var cleanMa = String(maBt).trim().toLowerCase();
    for (var i = 1; i < dataGiao.length; i++) {
      if (String(dataGiao[i][0]).trim().toLowerCase() === cleanMa) {
        return { error: "Mã bài tập này đã tồn tại trên hệ thống!" };
      }
    }
    ...
    sheetGiao.appendRow([maBt, monHoc, tenBaiTap, ngayPhatHanh, hanNop, fileUrl]);
    var lastRow = sheetGiao.getLastRow();
    sheetGiao.getRange(lastRow, 1, 1, 6).setFontFamily("Arial");
    
    return { success: true, link: fileUrl };
  } catch (e) {
    return { error: "Lỗi giao bài tập: " + e.toString() };
  } finally {
    lock.releaseLock();
  }
}

// Lấy danh sách các bài làm đã nộp của một học sinh (cho gia sư xem)
function layDanhSachHocSinhNop(studentPhone) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return [];
    ...
    for (var i = 1; i < dataNop.length; i++) {
      ...
      if (phone === normPhone && deletedDate === "") {
        list.push({
          rowIndex: i + 1,
          stt: dataNop[i][0],
          maBt: dataNop[i][1],
...
// Lấy danh sách các bài làm đã bị xóa (đưa vào thùng rác) của học sinh
function layDanhSachBaiNopDaXoa(sdtHs) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheetNop = ss.getSheetByName('Học sinh nộp bài');
    if (!sheetNop) return [];
