            ...
            if (saved) {
                try {
                    ...
                    if (currentTutorData.classes && Array.isArray(currentTutorData.classes) && currentTutorData.classes.length > 0) {
                        currentClasses = currentTutorData.classes.filter(function(c) { return c && c.classId && c.className; });
                        renderDashboardUI();
                    }
                    ...
                    if (res && res.classes && Array.isArray(res.classes) && res.classes.length > 0) {
                        ...
                        if (currentClasses && currentClasses.length > 0) {
                            currentClasses.forEach(function(localCls) {
                                if (localCls && localCls.classId && !merged.some(function(mc) { return mc && (mc.classId === localCls.classId || mc.className === localCls.className); })) {
                                    merged.push(localCls);
                                }
                            });
                        }
                        currentClasses = merged.filter(function(c) { return c && c.classId && c.className; });
                        ...
                        if (savedData) {
                            try {
                                ...
                                if (parsed && parsed.classes && parsed.classes.length > 0) {
                                    currentClasses = parsed.classes.filter(function(c) { return c && c.classId && c.className; });
